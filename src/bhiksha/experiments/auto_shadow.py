    ...
    *,
    ...
    for row in rows:
        ...
        opt = build_option_selection_contract(
            ...
            selector_version="1",
            parameters={
                "dte_min": int(getattr(exec_spec, "dte_min", 3)),
                "dte_max": int(getattr(exec_spec, "dte_max", 7)),
                "target_abs_delta_min": float(getattr(exec_spec, "target_abs_delta_min", 0.15)),
                "target_abs_delta_max": float(getattr(exec_spec, "target_abs_delta_max", 0.35)),
                "dte_fallback_policy": str(getattr(exec_spec, "dte_fallback_policy", "allow_nearest_after")),
                "max_bid_ask_spread_pct": float(getattr(exec_spec, "max_bid_ask_spread_pct", 0.08)),
                "min_open_interest": int(getattr(exec_spec, "min_open_interest", 100)),
