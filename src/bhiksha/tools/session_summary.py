...
def main(argv: list[str] | None = None) -> int:
    ...
    print("PENDING_EXIT_COUNTS=" + ",".join(f"{key}:{value}" for key, value in sorted(summary.pending_exit_counts.items())))
    print("AMBIGUOUS_CANCEL_COUNTS=" + ",".join(f"{key}:{value}" for key, value in sorted(summary.ambiguous_cancel_counts.items())))
