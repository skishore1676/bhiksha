...
from bhiksha.market_data.session import et_time_expr
...
class TimeframeResampler:
    """Resample OHLCV bars and align higher-timeframe features."""
    ...
    def filter_market_hours(
        ...
        *,
        ...
        before = len(df)
        filtered = df.filter(
            (et_time_expr(self.timestamp_col) >= market_open_time)
            & (et_time_expr(self.timestamp_col) <= market_close_time)
        )
    ...
    def join_timeframe_features(
        ...
        *,
        ...
        join_key = f"ts_{tag}"
        selected = [self.timestamp_col, *[column for column in feature_columns if column in timeframe_df.columns]]
