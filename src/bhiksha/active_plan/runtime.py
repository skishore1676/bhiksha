...
class BhikshaRuntime:
    ...
    async def run_session(
        ...
        *,
        ...
        session_id = (
            f"{active_plan_id}:{startup_config_id}"
            if active_plan_id and startup_config_id
            else None
        )
        for deployment in self.deployments:
            ...
            deployment_evidence_identity[deployment.deployment_id] = {
                ...
                "canary_start_at": _optional_identity_text(
                    metadata.get("canary_start_at")
                ),
                "canary_expires_at": _optional_identity_text(
                    metadata.get("canary_expires_at")
                ),
                "plan_revision_id": plan_revision_id,
                "session_id": session_id,
                "stop_loss_pct": deployment.risk.stop_loss_pct,
                "round_trip_cost_per_contract_usd": (
                    raw_policy.get("round_trip_cost_per_contract_usd")
                    if isinstance(raw_policy := metadata.get("canary_policy"), dict)
                    else None
                ),
            }
        ...
        try:
            ...
            # PRE-WARMUP TOKEN HEALTH CHECK (near-term fix) — autonomous: refresh_token_near_expiry
            # is still tradable (guard already refreshed access_token, will alert via Bus/Telegram for renewal).
            # Block only on truly expired/missing/invalid.
            if "schwab" in {self.provider_config.underlying_live_primary, warmup_provider}:
                ...
                if token_snapshot.state not in {"healthy", "access_token_stale", "refresh_token_near_expiry"}:
                    raise RuntimeError(
                ...
                if token_snapshot.state == "refresh_token_near_expiry":
                    output("TOKEN_HEALTH Schwab authentication valid for session — refresh soon (alerted via Lathi Bus)")
                else:
