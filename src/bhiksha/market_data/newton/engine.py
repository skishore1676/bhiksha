"""Compatibility facade over the Newton feature pipeline."""
...
from bhiksha.market_data.newton.transforms import (
...
DEFAULT_VPOC_LOOKBACK = 240
DEFAULT_EMA_PERIODS = (4, 8, 12)
DEFAULT_VOLUME_MA_PERIOD = 20
DEFAULT_VMA_LENGTH = 10
DEFAULT_VWMA_PERIODS = (8, 21, 34)


class PhysicsEngine:
    """Apply Newton transforms to raw market data."""

    def __init__(
        self,
        vpoc_lookback: int = DEFAULT_VPOC_LOOKBACK,
        ema_periods: Sequence[int] | None = None,
        volume_ma_period: int = DEFAULT_VOLUME_MA_PERIOD,
        vma_length: int = DEFAULT_VMA_LENGTH,
        vwma_periods: Sequence[int] | None = None,
        transforms: Sequence[FeatureTransform | str] | None = None,
    ) -> None:
        self.vpoc_lookback = vpoc_lookback
        self.ema_periods = tuple(ema_periods or DEFAULT_EMA_PERIODS)
        self.volume_ma_period = volume_ma_period
        self.vma_length = vma_length
        self.vwma_periods = tuple(vwma_periods or DEFAULT_VWMA_PERIODS)
    ...
    def _market_impulse_transforms_for_features(self, required_features: set[str]) -> list[FeatureTransform]:
        ...
        for feature in required_features:
            ...
            if vma_match:
                ...
                length = int(vma_match.group("vma_length"))
                if timeframe:
                    add_request(timeframe, length)
                else:
                    base_requested = True
                    base_lengths.add(length)
        ...
        for timeframe, lengths in sorted(requests.items()):
            ...
            transforms.append(
                MarketImpulseTransform(
                    vma_length=next(iter(lengths), self.vma_length),
    ...
    def _resolve_transforms(self, transforms: Sequence[FeatureTransform | str]) -> list[FeatureTransform]:
