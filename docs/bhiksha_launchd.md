...
| `com.bhiksha.live-stop` | Weekdays 15:10 CT | Stop the live runtime, then write a read-only, source-owned chart-evidence packet to `artifacts/chart-workbench/publications/`. It does not skip non-trading days, so stale processes can still be cleaned up. The export is attempted only after the stop stage succeeds; an export failure is visible in the job receipt and cannot start or trade the runtime. |
