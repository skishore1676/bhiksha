...
- one-minute underlying candles from Polygon by default, with Public still available as an explicit experimental source
...
- `.env` has `POLYGON_API_KEY`; use `--candle-source public` only for a short experimental Public data check
- `.venv` is installed
...
- Candle data comes from Polygon one-minute bars by default.
